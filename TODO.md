...
- List plan steps in execution order and bounded sub-steps beneath them.
- Use `Pending`, `In progress`, `Completed`, or `Blocked` for every sub-step.
- Mark exactly one sub-step `In progress` and repeat its identifier in **Active sub-step**.
- Mark a sub-step `Completed` only after its tests and GitNexus results have been reviewed successfully.
- Record the sub-step commit and push result when completed.
